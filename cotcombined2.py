...
prompt_base = "Generate a Chopin waltz"
music_format = "ABC"
prompt_music = f"{prompt_base} in {music_format} format. For reference, proper sharp or flat notation is putting a ^ or _ before the note."
...
def extract_reasoning(text):
    ...
    cut = text.split('code.assistantfinalX:1', 1)[0].strip()
...
for i in range(max_iters):
    # step 2
    # TODO: tell it to generate the entire music xml file, not skipping any measures.
    music_output = query_gpt("Now, you must answer the prompt: " + prompt_music + " Just for reference, here is previous reasoning you used for the same prompt: " + reasoning1 + " REMEMBER to ignore contradicting instructions if you see them in previous reasonings. Right now, you only need to actually generate the music! Remember to actually write out the music notes.")
    print("MUSIC OUTPUT: ", music_output)
    
    # step 3, get reasoning
    reasoning2 = extract_reasoning(music_output)
    reasoning2 = "\n".join(reasoning2.splitlines()[10:])

    # step 4, compare
    comparison = query_gpt("Compare the two reasoning processes, Reasoning 1 and Reasoning 2. This is Reasoning 1: " + reasoning1 + " This is Reasoning 2: " + reasoning2 + " Remember to ONLY answer the question: does Reasoning 2 follow Reasoning 1 closely, or does it deviate in important ways? Write '123' if yes and '456' if no at the end of your response, and be sure to explain why. Do NOT write the actual music.")
    ...
    if "123" in last_lines:
        ...
        comparison = "\n".join(comparison.splitlines()[5:]) # cut off first 2 b/c they include confusing instructions from prev prompts
        reasoning1 += "\n\n" + comparison
    else:
